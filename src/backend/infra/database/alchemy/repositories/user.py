...
class UserRepositoryImpl(BoundRepository[entity.User]):
    @as_result()
    async def get_one(self, **filters: Unpack[FilterOneUser]) -> dto.user.UserPrivate | None:
        result = await self._dao.get_one(for_update={}, **filters)

        return dto.user.UserPrivate.from_attributes(result) if result else None
    ...
    async def get_many_by_offset(
        ...
        **filters: Unpack[FilterManyUser],
    ) -> pagination.OffsetPaginationResult[dto.user.UserPrivate]:
        ...
        return pagination.OffsetPaginationResult(
            items=[dto.user.UserPrivate.from_attributes(item) for item in result.items],
    ...
    @as_result()
    async def create(self, data: CreateUserData) -> dto.user.UserPrivate | None:
    ...
    ) -> dto.user.UserPrivate | None:
        result = await self._dao.update(data.as_dict(), **filters)

        return dto.user.UserPrivate.from_attributes(result[-1]) if result else None

    @as_result()
    async def delete(self, **filters: Unpack[FilterOneUser]) -> dto.user.UserPrivate | None:
