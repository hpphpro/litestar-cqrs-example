    ...
    *buses: type[QCBusType],
    ...
    for func in chain.from_iterable(get_overloads(bus.send_unwrapped) for bus in buses):
        ...
        if origin is not AwaitableProxy:
            raise TypeError(f"Return type must be a type of {AwaitableProxy}.")

        args = get_args(proxy)
        if not args:
            raise TypeError(f"{AwaitableProxy} must have generic parameter")

        handler = args[0]
        if not issubclass(handler, Handler):
            raise TypeError(f"handler must inherit from base {Handler}.")
...
class BusBuilder:
    ...
    def buses[T: type[QCBusType] | Any](self, *buses: T) -> BusBuilder:
        self._buses.extend(buses)
