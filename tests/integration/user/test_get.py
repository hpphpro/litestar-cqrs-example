...
async def test_get_many_by_offset_user_failed(client: AsyncTestClient[Litestar]) -> None:
    response = await client.get("/api/v1/users", params={"page": 1, "limit": 10})
...
async def test_get_one_user_success(client: AsyncTestClient[Litestar]) -> None:
...
async def test_get_one_user_failed(client: AsyncTestClient[Litestar]) -> None:
    response = await client.get(f"/api/v1/users/{uuid.uuid4()}")

    assert response.status_code == 404


async def test_get_many_by_offset_user_success(client: AsyncTestClient[Litestar]) -> None:
    for i in range(10):
    ...
    response = await client.get("/api/v1/users", params={"page": 2, "limit": 10})
